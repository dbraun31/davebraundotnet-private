...
I accurately predicted incident COVID-19 cases up to three weeks in the future
by incorporating community perceptions of compliance with non-pharmaceutical
interventions (eg, social distancing) into probabilistic forecast models. My
work showed that these community perceptions can be included in a computational
model that could provide real-time forecasts of epidemiological targets to
...
```{r fig.cap="Distraction is an easy way to avoid feeling anxiety.", echo=FALSE, fig.align='center', out.width="55%"}
...
```{r eeg, fig.cap="(A) Cartoonized illustration of EEG cap and (B) placement of EEG electrodes in the experiment according to the 10-20 coordinate system", echo=FALSE}

knitr::include_graphics('cap_sensors.png')
...
see *Figure* \@ref(fig:design)A).

```{r design, fig.cap="Trial sequence and analysis approach. (A) Participants were instructed to let their minds wander freely while viewing a blank screen and occasionally reported on aspects of their experience (such as their level of subjective arousal). (B) Brain EEG signals before a thought probe and directly after heartbeats were averaged together, separated by a median split in subjective arousal, and submitted to a whole-brain analysis (ie, cluster-based permutation analysis).", out.width="100%", echo=FALSE, fig.align='left'}
...
directly after a heartbeat (see *Figure* \@ref(fig:design)*B*). I then separated
...
cluster's channels. Below (*Figure* \@ref(fig:heatmap)) is a heat map of the t values across all data points
(with a preview of a signifcant cluster):

```{r heatmap, fig.cap="Heat map of t values from cluster-based permutation analysis across time and EEG channels. Time is relative to heartbeat onset. First letter of channel suggests it's position on the scalp (ie, F=frontal, C=central, T=temporal, P=parietal, O=occipital). Darker blues indicate higher t values, revealing a larger difference in the EEG signal between subjective arousal conditions. Green dashed lines highlight the significant cluster."}
# Read in data
...
about the future). *Figure* \@ref(fig:correlations) shows participant-specific 
correlations between subjective arousal and both heartrate and future thinking.

```{r correlations, fig.cap="Participant-level correlations between subjective arousal, tendency to think about the future, and heartrate. Bayes factors and p values characterize a one-sample t test against zero.", cache=FALSE}
# Import data
...
*Figure* \@ref(fig:clusterresult) highlights the significant cluster from the cluster-based permutation analysis. We can see that, on average, EEG voltage was higher (ie, stronger interoception) prior to lower ratings of subjective arousal. This suggests that participants were more sensitive to cardiac signals during periods of lower subjective arousal. 
...
positive correlation with HEP effect size (<i>Figure</i> \@ref(fig:anxietycor)). This
